...
class Parser:
	...
	def if_expr_c(self):
		...
		if self.current_tok.matches(T_KEYWORD, 'ELSE'):
			...
			if self.current_tok.type == T_NEWLINE:
				...
				if self.current_tok.matches(T_KEYWORD, '}'):
					res.register_advancement()
					self.advance()
				else:
					return res.failure(InvalidSyntaxError(
	...
	def if_expr_b_or_c(self):
		...
		if self.current_tok.matches(T_KEYWORD, 'ELSE IF'):
	...
	def func_def(self):
		...
		if not self.current_tok.matches(T_KEYWORD, 'FUNCTION'):
			return res.failure(InvalidSyntaxError(
				self.current_tok.pos_start, self.current_tok.pos_end,
				f"Expected 'FUNCTION'"
			))
	...
	def atom(self):
		...
		elif tok.matches(T_KEYWORD, 'FUNCTION'):
		...
		return res.failure(InvalidSyntaxError(
			...
			"Expected num, identifier, '+', '-', '(', '[', 'IF', 'FOR', 'WHILE', 'FUNCTION'"
		))
	...
	def comp_expr(self):
		...
		node = res.register(self.bin_op(self.arith_expr, (T_EE, T_GT, T_LT, T_GTE, T_LTE, T_NE)))

		if res.error: 
			return res.failure(InvalidSyntaxError(
				self.current_tok.pos_start, self.current_tok.pos_end,
				"Expected num, identifier, '+', '-', '(', '[', 'IF', 'FOR', 'WHILE', 'FUNCTION' or 'not'"
			))
...
class BuiltInFunction(BaseFunction):
	...
	def execute_print(self, exec_ctx):
		print(str(exec_ctx.symbol_table.get('val')))
		return RTResult().success(Number.null)
	execute_print.arg_names = ['val']

	def execute_print_ret(self, exec_ctx):
		return RTResult().success(String(str(exec_ctx.symbol_table.get('val'))))
	execute_print_ret.arg_names = ['val']

	def execute_input(self, exec_ctx):
		text = input(exec_ctx.symbol_table.get('msg'))
		return RTResult().success(String(text))
	execute_input.arg_names = ['msg']
	...
	execute_input_num.arg_names = ['msg']
	...
	def execute_is_num(self, exec_ctx):
		is_num = isinstance(exec_ctx.symbol_table.get('val'), Number)
		return RTResult().success(Number.true if is_num else Number.false)
	execute_is_num.arg_names = ['val']

	def execute_is_string(self, exec_ctx):
		is_str = isinstance(exec_ctx.symbol_table.get('val'), String)
		return RTResult().success(Number.true if is_str else Number.false)
	execute_is_string.arg_names = ['val']

	def execute_is_list(self, exec_ctx):
		is_list = isinstance(exec_ctx.symbol_table.get('val'), List)
		return RTResult().success(Number.true if is_list else Number.false)
	execute_is_list.arg_names = ['val']

	def execute_is_function(self, exec_ctx):
		is_func = isinstance(exec_ctx.symbol_table.get('val'), BaseFunction)
		return RTResult().success(Number.true if is_func else Number.false)
	execute_is_function.arg_names = ['val']
...
BuiltInFunction.is_num = BuiltInFunction('num')
...
global_symbol_table.set("PHI", Number.phi)
